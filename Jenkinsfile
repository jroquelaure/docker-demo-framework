...
sed -ie "s/ubuntu:5001/${dockerRepo}/g" docker-framework/framework-test/Dockerfile'''
    ...
    stage('Push image to Artifactory') {
      steps {
        ...
        sh '"""curl -H \'X-JFrog-Art-Api:${apiKey}\' ${server}.url/api/docker/docker-dev-local/v2/promote -H "Content-Type:application/json" -d {"targetRepo" : "docker-dev-local", "dockerRepository" : "docker-framework", "tag" : 3${env.BUILD_ID}", "targetTag" : "latest", "copy": true }\' """ '
      }
    }
    stage('Test Image') {
      steps {
